...
// ===================================
// Comprehensive business intelligence + AI Revenue Attribution
...
async function getAnalyticsData() {
  ...
  // This month's jobs - use is_spam field since jobs table uses different schema
  ...
  // This month's payments - table may not exist
  ...
  // Last month's payments - table may not exist
  ...
  // Derive unique customers from jobs table instead of customers table
  ...
  (allJobs || []).forEach(job => {
    if (job.phone_number && !uniquePhones.has(job.phone_number)) {
      uniquePhones.add(job.phone_number);
      customerDates.push(new Date(job.created_at));
    }
  });

  const thisMonthCustomers = customerDates.filter(d => d >= startOfMonth).length;
  const lastMonthCustomers = customerDates.filter(
    d => d >= startOfLastMonth && d <= endOfLastMonth
  ).length;

  // Calls and messages - tables may not exist, handle gracefully
  ...
  const { count: callCount, error: callError } = await supabase
    ...
    .gte('created_at', startOfMonth.toISOString());
  if (!callError) callsThisMonth = callCount || 0;

  const { count: msgCount, error: msgError } = await supabase
    .from('message_logs')
    .select('*', { count: 'exact', head: true })
    .gte('created_at', startOfMonth.toISOString());
  if (!msgError) messagesThisMonth = msgCount || 0;
  ...
  const { data: aiRevenueData } = await supabase
    .from('ai_revenue')
    .select('source, amount')
    .gte('created_at', startOfMonth.toISOString());

  if (aiRevenueData) {
    aiRevenueData.forEach(record => {
      if (record.source === 'phone_booking') {
        aiRevenue.phoneBookings += record.amount || 0;
      } else if (record.source === 'quote_accepted') {
        aiRevenue.quoteConversions += record.amount || 0;
      }
      aiRevenue.total += record.amount || 0;
    });
  }
  ...
  const quotes = {
    ...
    sent: quoteStats?.filter(q => ['sent', 'viewed', 'accepted'].includes(q.status)).length || 0,
  ...
  const invoices = {
    ...
    pending: invoiceStats?.filter(i => ['sent', 'viewed'].includes(i.status)).length || 0,
  };
      ...
      <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="p-4 bg-purple-50 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-purple-700">Phone AI Bookings</span>
              </div>
              <div className="text-2xl font-bold text-purple-900">{formatCents(data.aiRevenue.phoneBookings * 100)}</div>
              <p className="text-xs text-purple-600 mt-1">From AI-handled calls</p>
            </div>
            <div className="p-4 bg-green-50 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <FileText className="w-5 h-5 text-green-600" />
                <span className="text-sm font-medium text-green-700">Quote Conversions</span>
              </div>
              <div className="text-2xl font-bold text-green-900">{formatCents(data.aiRevenue.quoteConversions * 100)}</div>
              <p className="text-xs text-green-600 mt-1">From AI-generated quotes</p>
            </div>
            <div className="p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <Zap className="w-5 h-5 text-blue-600" />
                <span className="text-sm font-medium text-blue-700">Total AI Revenue</span>
              </div>
              <div className="text-2xl font-bold text-blue-900">{formatCents(data.aiRevenue.total * 100)}</div>
              <p className="text-xs text-blue-600 mt-1">All AI-attributed revenue</p>
            </div>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-2 gap-6">
          <Card className="p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                ...
                <span className="font-semibold text-slate-900">{data.quotes.total}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                <span className="text-slate-600">Sent to Customers</span>
                <span className="font-semibold text-slate-900">{data.quotes.sent}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                <span className="text-slate-600">Accepted</span>
                <span className="font-semibold text-green-600">{data.quotes.accepted}</span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-slate-600">Conversion Rate</span>
                <span className="font-semibold text-blue-600">
                  {data.quotes.sent > 0 ? Math.round((data.quotes.accepted / data.quotes.sent) * 100) : 0}%
                </span>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                <span className="text-slate-600">Total Created</span>
                <span className="font-semibold text-slate-900">{data.invoices.total}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                <span className="text-slate-600">Pending Payment</span>
                <span className="font-semibold text-yellow-600">{data.invoices.pending}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
              ...
              <div className="flex justify-between items-center py-2">
                ...
                <span className="font-semibold text-blue-600">
                  {data.invoices.total > 0 ? Math.round((data.invoices.paid / data.invoices.total) * 100) : 0}%
                </span>
              </div>
            </div>
          </Card>
